...
class RRT:
    ...
    def __init__(self, fr, is_in_collision):
        ...
        '''
        self._project_step_size = 1e-2
        self._constraint_th = 2e-3 #(< 2e-3)

        self._q_step_size = 0.08 #( < 0.1)
        self._target_p = 0.15
    ...
    def sample_valid_joints(self):
        ...
        for i in range (self._fr.num_dof):
            q[i] = (self._fr.joint_limits_high[i] - self._fr.joint_limits_low[i]) * np.random.random_sample(1) + self._fr.joint_limits_low[i]
        return q
    ...
    def project_to_constraint(self, q, constraint):
        ...
        q_proj = q.copy()
        learning_rate = self._project_step_size
        threshold = self._constraint_th
        err,gradient = constraint(q_proj)

        while err > threshold:

            jacob = self._fr.jacobian(q_proj)
            q_proj = q_proj - learning_rate * jacob.T @ np.array(gradient)
            err,gradient = constraint(q_proj)

        return q_proj

    def extend(self, tree, q_target, constraint):
        '''
        ...
        '''
        #constraint is a "function" instead of a value
        ...
        while True:
            ...
            q_near_id, dist = tree.get_nearest_node(q_sample)
            q_near = tree.get_point(q_near_id)

            q_new = q_near + np.minimum(self._q_step_size, np.linalg.norm(q_sample - q_near)) * (q_sample - q_near) / np.linalg.norm(q_sample - q_near)
            ...
            if self._is_in_collision(q_new):
                continue
            new_node_id = tree.insert_new_node(q_new, q_near_id)
