...
if __name__ == '__main__':
    ...
    def ee_upright_constraint(q):
        ...
        ee = fr.ee(q)
        roll, pitch = ee[3:5]
        des_roll = desired_ee_rp[0]
        des_pitch = desired_ee_rp[1]
        err = (des_roll-roll)* (des_roll-roll) + (des_pitch-pitch)*(des_pitch-pitch)
        grad = [0,0,0,-2 * (des_roll - roll),-2 * (des_pitch - pitch) ,0]
    ...
    constraint = ee_upright_constraint #make an alias of the function
